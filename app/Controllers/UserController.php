...
class UserController extends Controller
{
    ...
    public function registerPost()
    {
        ...
        if ((isset($_POST["register"])) && (!empty($_POST))) {
            ...
            if (empty($nom && $email && $password && $phone && $adress)) {
                ...
                $error =  "Le mot de passe doit être de 8 caractères minimum et contenir au moins <span>(1 chiffre, 1 caractère majuscule et minuscule, 1 caractère spécial)</span>";
                include VIEWS . 'auth/register.php';
                $content = ob_get_clean();
                require VIEWS . 'layout.php';
            } else if (empty($error)) {
    ...
    public function loginPost()
    {
        ...
        $user = (new User($this->getDB()))->checkEmail($_POST['email']);
        $password = $_POST['password'];
        $check_Password = $user->checkPassword($password);
